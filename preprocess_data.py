...
class ImagePreprocessor():
    ...
    def save_data(self, path, prefix):
        ...
        for i, (img, lanmark) in enumerate(zip(self.imgs, self.landmarks)):
            ...
            save_path = os.path.join(path, prefix+'_'+str(i)+'.png').replace('\\','/')
...
def preprocess_dataset(img_dir, out_dir, landmark_file, is_train):

    with open(landmark_file,'r') as f:
        lines = f.readlines()
        labels = []
        save_img = os.path.join(out_dir, 'imgs')
        ...
        for i, line in tqdm(enumerate(lines), total=len(lines)):
            Img = ImagePreprocessor(line, img_dir)
            img_name = Img.path
...
if __name__ == '__main__':
    ...
    image_dir = data_dir + '/WFLW/WFLW_images'
    label_dir = data_dir + '/WFLW/WFLW_annotations'
    Mirror_file = label_dir + '/Mirror98.txt'

    landmark_dir = label_dir + '/list_98pt_rect_attr_train_test'
    landmark_files = [
        landmark_dir + f'/list_98pt_rect_attr_{s}.txt' for s in ['test','train']
    ]
    output_dirs = ['test_data', 'train_data']

    for landmark_file, out_dir in zip(landmark_files, output_dirs):

        out_dir = os.path.join(data_dir, out_dir).replace('\\', '/')
        print('\n\n', landmark_file, '-->', out_dir)

        if os.path.exists(out_dir):
            shutil.rmtree(out_dir)
        os.mkdir(out_dir)

        if 'list_98pt_rect_attr_test.txt' in landmark_file:
            is_train = False
        else:
            is_train = True

        preprocess_dataset(image_dir, out_dir, landmark_file, is_train)
